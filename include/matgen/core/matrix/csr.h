#ifndef MATGEN_CORE_MATRIX_CSR_H
#define MATGEN_CORE_MATRIX_CSR_H
 ...
 *
 * Allocates memory for row_ptr, col_indices, and values using automatic
 * backend selection. Arrays must be filled by the caller.
 *
...
/**
 * @brief Create a new CSR matrix with explicit execution policy
 *
 * @param rows Number of rows
 * @param cols Number of columns
 * @param nnz Number of non-zeros
 ...
 * @return Pointer to new matrix, or NULL on error
 */
matgen_csr_matrix_t* matgen_csr_create_with_policy(matgen_index_t rows,
                                                   matgen_index_t cols,
                                                   matgen_size_t nnz,
...
#endif  // MATGEN_CORE_MATRIX_CSR_H

...
// =============================================================================
// COO to CSR Conversion (Sequential)
// =============================================================================
...
matgen_csr_matrix_t* matgen_coo_to_csr_seq(const matgen_coo_matrix_t* coo) {
  ...
  MATGEN_LOG_DEBUG("Converting COO (%llu x %llu, nnz=%zu) to CSR (SEQ)",
                   (unsigned long long)coo->rows, (unsigned long long)coo->cols,
                   coo->nnz);

  // Create CSR matrix using sequential backend explicitly
  matgen_csr_matrix_t* csr = matgen_csr_create_with_policy(
      coo->rows, coo->cols, coo->nnz, MATGEN_EXEC_SEQ);
  if (!csr) {
    return NULL;
  }
  ...
  if (!coo->is_sorted) {
    ...
    // Sort the copy using sequential backend
    if (matgen_coo_sort_with_policy(coo_sorted, MATGEN_EXEC_SEQ) !=
        MATGEN_SUCCESS) {
  ...
  MATGEN_LOG_DEBUG("COO to CSR conversion complete (SEQ)");

  return csr;
}

// =============================================================================
// CSR to COO Conversion (Sequential)
// =============================================================================
...
matgen_coo_matrix_t* matgen_csr_to_coo_seq(const matgen_csr_matrix_t* csr) {
  ...
  MATGEN_LOG_DEBUG("Converting CSR (%llu x %llu, nnz=%zu) to COO (SEQ)",
  ...
  MATGEN_LOG_DEBUG("CSR to COO conversion complete (SEQ)");
